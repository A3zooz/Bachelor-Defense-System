...
def neighbor(solution,flag1):
    ...
    for Examiner in solution[1]:
            for day in range(12):
                ...
                if solution[6][Examiner] > 10:        
                    ...
                    if (temp1 < 3 and temp1 > 0) or  (temp1 > 0 and temp1 != solution[6][Examiner]) :
                        for k in range(len(solution[0])):
                            if(solution[0][k]["Examiner"]==Examiner and solution[0][k]["Time"]>=day*15 and solution[0][k]["Time"]<(day*15+15) ):
                                candidates.append((k,"maxAndMin"))

    #If examiner has 10 slots or less all of them must be in one day
    # for Examiner in solution[1]:
    ...
    #         for k in range(len(solution[0])):
    #             if(solution[0][k]["Examiner"]==Examiner and solution[0][k]["Time"]>=day*15 and solution[0][k]["Time"]<(day*15+15) ):
    #                 candidates.append((k,"lessThan12"))
    ...
    if "maxAndMin" in reason:
        selected_day = solution[0][i]['Time'] // 15
